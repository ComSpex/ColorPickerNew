...
namespace ColorPicker {
  public partial class ColorPickerCtrl : UserControl {
    ...
    public ColorPickerCtrl() {
      ...
      hue = 0;
      while (hue < 360) {
        colors.Add(new HSLColor(hue, 0.3, 0.5).Color);
        hue += huestep;
      }
      hue = 0;
      while (hue < 360) {
        colors.Add(new HSLColor(hue, 0.9, 0.5).Color);
        hue += huestep;
      }
      #endif
    ...
    void UpdateInfo() {
      ...
      string info = string.Format("{0} aRGB({1},{2},{3},{4}:#{5:X6})", m_colorWheel.SelectedHSLColor.ToString(), c.A, c.R, c.G, c.B,c.ToArgb());
